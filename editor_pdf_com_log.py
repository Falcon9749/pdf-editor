...
LOG_FILE = os.path.join(LOG_DIR, "app.log")  # arquivo principal; o handler cria versões diárias

logger = logging.getLogger()
logger.setLevel(logging.DEBUG)

# Cria arquivos de log diários
...
class PDFEditorApp:
    ...
    def prev_page(self):
        if self.doc and self.current_page > 0:
            self.current_page -= 1
            self.render_page()
            logger.info(f"Página anterior: {self.current_page+1}")

    def next_page(self):
        if self.doc and self.current_page < len(self.doc) - 1:
            self.current_page += 1
            self.render_page()
            logger.info(f"Próxima página: {self.current_page+1}")
    ...
    def zoom_in(self):
        ...
        self.render_page()
        logger.info(f"Zoom in: {self.scale}")
    ...
    def zoom_out(self):
        ...
        self.render_page()
        logger.info(f"Zoom out: {self.scale}")
    ...
    def on_click(self, event):
        ...
        if clicked_word and rect:
            ...
            if new_text:
                ...
                logger.info(f"Texto editado: '{clicked_word}' → '{new_text}'")
                self.render_page()
    ...
    def save_state(self):
        if self.doc:
            ...
            self.redo_stack.clear()
            logger.debug("Estado salvo para desfazer")
